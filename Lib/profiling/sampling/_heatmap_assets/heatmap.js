...
function calculateMenuPosition(buttonRect, menuWidth, menuHeight) {
  ...
  const left =
    buttonRect.right + menuWidth + 10 < viewport.width
      ? buttonRect.right + scroll.x + 10
      : Math.max(scroll.x + 10, buttonRect.left + scroll.x - menuWidth - 10);

  const top =
    buttonRect.bottom + menuHeight + 10 < viewport.height
...
function handleNavigationClick(button, e) {
  ...
  if (navMulti) {
    ...
    const title = button.classList.contains("caller")
      ? "Choose a caller:"
      : "Choose a callee:";
...
function getSampleCount(line) {
  ...
  if (colorMode === "self") {
    text = line
      .querySelector(".line-samples-self")
      ?.textContent.trim()
      .replace(/,/g, "");
  } else {
    text = line
      .querySelector(".line-samples-cumulative")
      ?.textContent.trim()
      .replace(/,/g, "");
  }
...
function buildScrollMarker() {
  ...
  const lineHeight = Math.min(
    Math.max(3, window.innerHeight / lines.length),
    10,
  );
  ...
  lines.forEach((line, index) => {
    ...
    const intensityClass =
      maxSamples > 0
        ? intensityToClass(samples / maxSamples) || "cold"
        : "cold";

    if (
      lineNumber === prevLine + 1 &&
      lastMark?.classList.contains(intensityClass)
    ) {
...
function applyLineColors() {
  ...
  lines.forEach((line) => {
    ...
    if (colorMode === "self") {
      ...
      intensity =
        parseFloat(line.getAttribute("data-cumulative-intensity")) || 0;
    }
...
function applyHotFilter() {
  ...
  lines.forEach((line) => {
    const selfSamples = line
      .querySelector(".line-samples-self")
      ?.textContent.trim();
    const cumulativeSamples = line
      .querySelector(".line-samples-cumulative")
      ?.textContent.trim();
...
function calculateHeatColor(intensity) {
  ...
  if (intensity > 0.3) {
    ...
    const hotColor = getComputedStyle(document.documentElement)
      .getPropertyValue("--span-hot-base")
      .trim();
    return `rgba(${hotColor}, ${alpha})`;
  } else if (intensity > 0) {
    // Cold spans: very subtle gray, almost invisible
    const coldColor = getComputedStyle(document.documentElement)
      .getPropertyValue("--span-cold-base")
      .trim();
...
function populateBytecodePanel(panel, button) {
  ...
  try {
    ...
    const specPct =
      totalSamples > 0
        ? Math.round((100 * specializedSamples) / totalSamples)
        : 0;
    const specializedCount = instructions.filter(
      (i) => i.is_specialized,
    ).length;
    ...
    panel
      .querySelectorAll(".bytecode-instruction[data-locations]")
      .forEach((instrEl) => {
    ...
    panel.innerHTML =
      '<div class="bytecode-error">Error loading bytecode</div>';
...
function toggleAllBytecode() {
  ...
  const someExpanded = Array.from(buttons).some((b) =>
